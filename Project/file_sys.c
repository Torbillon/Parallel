...
file_hand_array_t init_file_hand_array(char* file_name, int* file_count, int rank, int nproc, int line_count) {
  ...
  if(num_blocks != 0) {
    ...
    disp = NUM_CHAR_LINE * rank * sizeof(char);
  ...
  for(i = 0; i < file_count; i++) {
    ...
    while(fgets(buf, FILE_READER_BUF_SIZE, open_file) != NULL) {
      ...
      if(strcmp(curr_token, ATOM) == 0) {
        ev_start += sprintf(everything_else + ev_start, "%s ", curr_token);
        ...
        // X        
        curr_token = strtok(NULL, delimiter_characters);
        x_start += sprintf(x_coordinates + x_start, "%s\n", curr_token);
        
        // Y
        curr_token = strtok(NULL, delimiter_characters);
        y_start += sprintf(y_coordinates + y_start, "%s\n", curr_token);

        // Z
        curr_token = strtok(NULL, delimiter_characters);
        z_start += sprintf(z_coordinates + z_start, "%s\n", curr_token);
